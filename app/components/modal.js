  ...
  if(show){
    return (
      <div onClick={onClose} className=' fixed inset-0 bg-black bg-opacity-30 backdrop-blur-sm flex justify-center items-center'>
        <div className=' m-10 max-h-[500px] max-w-[600px] overflow-y-auto bg-zinc-800 p-9 rounded text-white grid grid-cols-1 items-center gap-5'>
          <h1 className=' font-bold text-center text-xl'>Product Details:</h1>
          <div className=' flex justify-center items-center'>
            <Image className='w-34 h-52 rounded' src={product.image} alt={product.title} width={200} height={300}/>
          </div>
          <h1 className=' text-center font-semibold'>{product.title}</h1>
          <h2 className=' text-center'><span className=' font-semibold'>Category:</span> {product.category}</h2>
